...
class LLMClient:
    ...
    def __init__(
        self,
        provider: str = "openai",
        model: str = "gpt-4o-mini",     # the newest OpenAI model is "gpt-4o" which was released May 13, 2024. do not change this unless explicitly requested by the user
    ...
    def _setup_openai(self):
        ...
        # Get API key from environment or raise error with helpful message
        api_key = os.getenv("OPENAI_API_KEY")
        if not api_key:
            raise ValueError("The OPENAI_API_KEY environment variable must be set. Please provide your OpenAI API key in the sidebar.")
    ...
    def _setup_local(self):
        ...
        try:
            ...
            raise RuntimeError(f"Failed to load local models: {str(e)}")
    ...
    def key_phrases(self, text: str) -> List[str]:
        ...
        if self.provider == "openai":
            ...
            line = resp.choices[0].message.content.strip() if resp.choices[0].message.content else ""
